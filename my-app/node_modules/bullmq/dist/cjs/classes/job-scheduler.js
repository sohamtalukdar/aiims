...
class JobScheduler extends queue_base_1.QueueBase {
    ...
    async getJobScheduler(id) {
        ...
        const schedulerAttributes = await client.hgetall(this.toKey('repeat:' + id));
        if (schedulerAttributes) {
            return Object.assign({ key: id, name: schedulerAttributes.name, endDate: parseInt(schedulerAttributes.endDate) || null, tz: schedulerAttributes.tz || null, pattern: schedulerAttributes.pattern || null, every: schedulerAttributes.every || null }, (schedulerAttributes.data || schedulerAttributes.opts
                ? {
                    template: this.getTemplateFromJSON(schedulerAttributes.data, schedulerAttributes.opts),
                }
                : {}));
        }
    }
